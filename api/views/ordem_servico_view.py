...
from ..services import ordem_servico_service, equipamento_service, log_service
...
class OrdemServicoList(Resource):
    ...
    def post(self):
        """
            ...
            de servico (triagem, diagnostico, triagem+diagnostico)
            Se não será um cadastro da ordem de servico
            (triagem, diagnostico, triagem+diagnostico)
        """
        body = request.json
        try:
            _id = body["_id"]
        ...
        if 'triagem' in body or 'diagnostico' in body:
            erro_validacao = ordem_servico_schema \
                .OrdemServicoSchema().validate(body)
        else:
            return error_response(
                'Ordem de servico necessita das chave ' +
                '"triagem" ou "diagnostico"')
        ...
        if not _id:
            novo_ordem_servico = ordem_servico_service.registrar_ordem_servico(
                body)
            return Response(
...
class OrdemServicoDetail(Resource):
    ...
    def get(self, _id):
        """
            ...
            conforme o id do documento repassado.
        """
        ordem_servico = ordem_servico_service.listar_ordem_servico_by_id(_id)
        if ordem_servico is None:
            return make_response(
                jsonify("Ordem de serviço não encontrada..."), 404)
        return Response(
            ordem_servico.to_json(),
            mimetype="application/json",
            status=200)
    ...
    def patch(self, _id):
        ...
        if ('triagem' in body and len(body['triagem']) != 0) or (
                'diagnostico' in body and len(body['diagnostico']) != 0):
            erro_validacao = ordem_servico_schema \
                .OrdemServicoSchema().validate(body)
        else:
            return error_response(
                'Ordem de servico necessita da chave triagem ' +
                'ou diagnostico com as atualizacoes')
